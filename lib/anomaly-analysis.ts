...
async function analysisRows(
  ...
  const result = await d1()
    ...
    .bind(userId, accountId, startDate, endDate)
    .all<DailyRow>();
  ...
  for (const row of result.results) {
    ...
    const current = entities.get(key) ?? {
      objectId: key,
      objectName: String(row.entityLabel ?? key),
    ...
    current.sales += num(row.sales);
    current.daily.set(String(row.reportDate), {
      cost: num(row.cost),
      sales: num(row.sales),
  ...
  return [...entities.values()]
    ...
    .map((entity) => {
      ...
      const latest = series.at(-1)!,
        previous = series.slice(0, -1);
      const average = (field: keyof typeof latest) =>
        previous.reduce((sum, item) => sum + item[field], 0) /
      ...
      return {
        ...
        metrics15d: {
          impressions: round(entity.impressions, 0),
          clicks: round(entity.clicks, 0),
          cost: round(entity.cost),
          purchases: round(entity.purchases),
          sales: round(entity.sales),
          ctr: round(
            entity.impressions ? (entity.clicks / entity.impressions) * 100 : 0,
          ),
        ...
        latestDay: Object.fromEntries(
          Object.entries(latest).map(([key, value]) => [key, round(value)]),
        ),
        previous14DayAverage: {
          cost: round(average("cost")),
          sales: round(average("sales")),
          purchases: round(average("purchases")),
          clicks: round(average("clicks")),
          impressions: round(average("impressions")),
        },
...
function parseModelJson(content: string): {
  ...
  const anomalies = Array.isArray(parsed.anomalies)
    ? parsed.anomalies.slice(0, 30).map((item) => {
        ...
        const severity = ["high", "medium", "low"].includes(
          ...
          ? (String(row.severity) as StoredAnomaly["severity"])
          : "medium";
        return {
    ...
    : [];
  return {
    summary: String(parsed.summary ?? "分析完成").slice(0, 2000),
    anomalies,
  };
}

async function analyzeKind(
  userId: string,
  accountId: string,
  analysisDate: string,
  startDate: string,
  ...
  const existing = await d1()
    .prepare(
      `SELECT id,status,model_name modelName,summary,anomalies_json anomaliesJson FROM ad_anomaly_analyses WHERE account_id=? AND analysis_date=? AND report_kind=?`,
    )
    .bind(accountId, analysisDate, kind)
  ...
  if (!force && existing?.status === "COMPLETED")
    return {
      ...
      anomalies: JSON.parse(
        String(existing.anomaliesJson ?? "[]"),
      ) as StoredAnomaly[],
      reused: true,
    };
  const id = String(existing?.id ?? crypto.randomUUID()),
    now = Date.now(),
    config = await modelConfigForUser(userId);
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(
        `模型分析失败 (${response.status}): ${(await response.text()).slice(0, 240)}`,
      );
    ...
    try {
      ...
      parsed = {
        summary: content.slice(0, 2000) || "模型分析完成，但返回了非结构化内容",
        anomalies: [
          {
            objectName: `${LABELS[kind]}整体`,
            severity: "medium",
            anomaly: "模型返回非结构化异常结论",
            reason: content.slice(0, 1200) || "模型未返回可解析内容",
            evidence: "已保存原始模型响应，后续分析会继续要求结构化 JSON",
          },
        ],
      };
    }
    await d1()
      .prepare(
        `UPDATE ad_anomaly_analyses SET status='COMPLETED',summary=?,anomalies_json=?,raw_response=?,error=NULL,completed_at=?,updated_at=? WHERE id=?`,
      )
      .bind(
        parsed.summary,
        JSON.stringify(parsed.anomalies),
    ...
    await d1()
      .prepare(
        `UPDATE ad_anomaly_analyses SET status='FAILED',error=?,updated_at=? WHERE id=?`,
      )
      .bind(message.slice(0, 1500), Date.now(), id)
      .run();
    return {
      reportKind: kind,
      status: "FAILED",
...
export async function runAnomalyAnalysis(
  ...
  const account = await d1()
    .prepare(
      `SELECT timezone,marketplace,region FROM accounts WHERE id=? AND user_id=?`,
    )
    .bind(accountId, userId)
    .first<{ timezone?: string; marketplace?: string; region?: string }>();
  if (!account) throw new Error("店铺不存在");
  const today = localDate(effectiveTimezone(account)),
    analysisDate = options.date ?? today,
    endDate = options.date ?? shiftDate(today, -1),
    startDate = shiftDate(endDate, -14),
    analyses = [];
  ...
  return {
    ...
    startDate,
    endDate,
    status: analyses.every((item) => item.status === "COMPLETED")
      ? "COMPLETED"
      : "FAILED",
    analyses,
  };
...
export async function anomalyHistory(
  userId: string,
  accountId: string,
  selectedDate?: string | null,
) {
  const datesResult = await d1()
    .prepare(
      `SELECT DISTINCT analysis_date analysisDate FROM ad_anomaly_analyses WHERE user_id=? AND account_id=? ORDER BY analysis_date DESC LIMIT 120`,
    )
    .bind(userId, accountId)
    .all<{ analysisDate: string }>();
  const dates = datesResult.results.map((item) => item.analysisDate),
    analysisDate = selectedDate || dates[0] || null;
  if (!analysisDate) return { dates, analysisDate: null, analyses: [] };
  const result = await d1()
    .prepare(
      `SELECT report_kind reportKind,start_date startDate,end_date endDate,model_name modelName,status,summary,anomalies_json anomaliesJson,error,completed_at completedAt FROM ad_anomaly_analyses WHERE user_id=? AND account_id=? AND analysis_date=? ORDER BY CASE report_kind WHEN 'campaign' THEN 1 WHEN 'keyword' THEN 2 ELSE 3 END`,
    )
    .bind(userId, accountId, analysisDate)
