...
export function ensureSchema(): Promise<void> {
  ...
  ready = (async () => {
    ...
    const sql = [
      ...
      `CREATE INDEX IF NOT EXISTS sessions_user_idx ON sessions(user_id)`, `CREATE INDEX IF NOT EXISTS tasks_due_idx ON tasks(status,next_run_at)`, `CREATE INDEX IF NOT EXISTS audit_user_idx ON audit_logs(user_id,created_at)`,
    ];
