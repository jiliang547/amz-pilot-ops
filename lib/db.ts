...
export function ensureSchema(): Promise<void> {
  ...
  ready = (async () => {
    ...
    const sql = [
      ...
      `CREATE INDEX IF NOT EXISTS sessions_user_idx ON sessions(user_id)`, `CREATE INDEX IF NOT EXISTS tasks_due_idx ON tasks(status,next_run_at)`, `CREATE INDEX IF NOT EXISTS audit_user_idx ON audit_logs(user_id,created_at)`, `CREATE INDEX IF NOT EXISTS attachments_user_idx ON attachments(user_id,created_at)`, `CREATE UNIQUE INDEX IF NOT EXISTS custom_skills_user_name_idx ON custom_skills(user_id,name)`, `CREATE UNIQUE INDEX IF NOT EXISTS report_jobs_request_idx ON report_jobs(user_id,account_id,request_fingerprint)`, `CREATE INDEX IF NOT EXISTS report_jobs_history_idx ON report_jobs(user_id,created_at)`, `CREATE UNIQUE INDEX IF NOT EXISTS report_files_job_part_idx ON report_files(report_job_id,part_number)`, `CREATE UNIQUE INDEX IF NOT EXISTS report_snapshots_window_idx ON report_snapshots(account_id,report_type,window_key,snapshot_date)`, `CREATE INDEX IF NOT EXISTS report_snapshots_lookup_idx ON report_snapshots(user_id,account_id,status,snapshot_date)`, `CREATE UNIQUE INDEX IF NOT EXISTS ad_daily_facts_key_idx ON ad_daily_facts(account_id,report_date,campaign_id,ad_group_id)`, `CREATE INDEX IF NOT EXISTS ad_daily_facts_range_idx ON ad_daily_facts(user_id,account_id,report_date)`, `CREATE INDEX IF NOT EXISTS ad_daily_facts_sync_idx ON ad_daily_facts(account_id,sync_id)`, `CREATE UNIQUE INDEX IF NOT EXISTS ad_data_syncs_account_date_idx ON ad_data_syncs(account_id,sync_date)`, `CREATE INDEX IF NOT EXISTS ad_data_syncs_status_idx ON ad_data_syncs(user_id,account_id,status,updated_at)`,
    ];
    await db.batch(sql.map(s => db.prepare(s)));
    for (const statement of [`ALTER TABLE accounts ADD COLUMN marketplace TEXT`, `ALTER TABLE accounts ADD COLUMN timezone TEXT`, `ALTER TABLE accounts ADD COLUMN currency TEXT`]) {
      try { await db.prepare(statement).run(); } catch { /* Existing column. */ }
    }
    const now = Date.now();
    await db.batch([
