...
async function runReportKindSync(row: Record<string, unknown>, credentials: AmazonCredentials, client: AmazonAdsApiClient, kind: AmazonAdsReportKind, force: boolean, onStatus?: (text: string) => void, timeoutMs = 60 * 60_000, forceInitial = false): Promise<KindSyncResult> {
  ...
  if (!sync) await d1().prepare(`INSERT INTO ad_report_syncs(id,user_id,account_id,sync_date,report_kind,mode,start_date,end_date,status,rows_upserted,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`).bind(syncId, userId, accountId, syncDate, kind, mode, startDate, endDate, "RUNNING", 0, now, now).run();
  else await d1().prepare(`UPDATE ad_report_syncs SET mode=?,start_date=?,end_date=?,status='RUNNING',error=NULL,updated_at=? WHERE id=?`).bind(mode, startDate, endDate, now, syncId).run();
  try {
...
function insightRows(payload: SnapshotPayload) { const mapped = payload.groups.map(group => { const a = group.aggregates; return { label: group.label || "未命名", secondary: group.secondary, campaignName: group.campaignName, metrics: { ...a, roas: a.totalCost ? (a.sales ?? 0) / a.totalCost : 0, acos: a.sales ? (a.totalCost ?? 0) / a.sales * 100 : 0, ctr: a.impressions ? (a.clicks ?? 0) / a.impressions * 100 : 0 } }; }); return { topBySales: [...mapped].sort((a, b) => b.metrics.sales - a.metrics.sales).slice(0, 8), highSpendNoOrder: mapped.filter(item => item.metrics.totalCost > 0 && item.metrics.purchases === 0).sort((a, b) => b.metrics.totalCost - a.metrics.totalCost).slice(0, 8) }; }
export async function dashboardData(userId: string, accountId: string) {
  ...
  return { windows, anomalies, keywordInsights, searchTermInsights, reportSyncs: syncRows.results, generatedAt: Date.now() };
}